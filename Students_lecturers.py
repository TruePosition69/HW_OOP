...
def average(self):
      marks_list = [mark for marks in self.marks.values() for mark in marks]
      if marks_list:
...
mentor1 = Mentor('Иван', 'Иванов')
mentor1.courses_attached = ['Python', 'C##']
mentor1.marks = {'Python': [9, 9, 10], 'C##' : [7, 8, 8]}
    ...
    for student in students:
        if course_name in student.courses_in_progress or course_name in student.finished_courses:
          if course_name in student.grades:
              ...
              num_students += len(student.grades[course_name])
    if num_students == 0:
        return 0
...
def avg_marks_by_lecterer(lecturers, course_name):
    ...
    for lecturer in lecturers:
        if course_name in lecturer.courses_attached:
            if course_name in lecturer.marks:
                total_grade += sum(lecturer.marks[course_name])
                num_lecturers += len(lecturer.marks[course_name])
    if num_lecturers == 0:
        return 0
